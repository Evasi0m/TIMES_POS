-- 047_tiktok_confirm_defer_net.sql
-- Allow "ใส่ทีหลัง" when confirming TikTok orders at POS:
--   p_net_received IS NULL → net_received_pending = true (PendingNetBell queue)
--   p_net_received set     → net_received_pending = false
-- Backfill orders already confirmed without net but flagged incorrectly.

-- ====================================================================
-- 1. confirm_tiktok_sale_order — defer net via NULL p_net_received
-- ====================================================================
...
-- ====================================================================
-- 2. Backfill — confirmed TikTok orders missing net but not in bell queue
-- ====================================================================
UPDATE sale_orders
   SET net_received_pending = true,
       updated_at           = now()
 WHERE channel = 'tiktok'
   AND tiktok_order_id IS NOT NULL
   AND status = 'active'
   AND confirmed_at IS NOT NULL
   AND net_received IS NULL
   AND net_received_pending = false;
