-- 063: Backfill sale_order_items for active TikTok orders that have zero lines.
--
-- Gap: re-import calls upsert_tiktok_order_fulfillment which only UPDATEs existing
-- lines. Active/confirmed orders that somehow have 0 rows never get lines inserted
-- on sync. resync_tiktok_pending_order only runs for status = 'pending'.

CREATE OR REPLACE FUNCTION public.backfill_tiktok_order_lines_if_empty(
  p_order_id bigint,
  p_items    jsonb
) RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  IF jsonb_typeof(p_items) <> 'array' OR jsonb_array_length(p_items) = 0 THEN RETURN; END IF;
  IF EXISTS (SELECT 1 FROM sale_order_items WHERE sale_order_id = p_order_id LIMIT 1) THEN
    RETURN;
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    SELECT
      ...
      0, 'amount', 0, 'amount',
      p.cost_price,
      NULLIF(v_item->>'tiktok_sku_id', ''),
      NULLIF(v_item->>'seller_sku', ''),
      NULLIF(v_item->>'sku_name', ''),
      NULLIF(v_item->>'sku_image_url', ''),
      NULLIF(v_item->>'tiktok_line_id', '')
...
REVOKE ALL ON FUNCTION public.backfill_tiktok_order_lines_if_empty(bigint, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.backfill_tiktok_order_lines_if_empty(bigint, jsonb) TO service_role;

-- Hook into import_tiktok_sale_order re-import path (051 body + backfill call).
