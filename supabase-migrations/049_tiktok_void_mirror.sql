-- 049_tiktok_void_mirror.sql — mirror POS stock → TikTok after void / delete receive line

-- ── sync_operation column (receive | void) ──────────────────────────────────
ALTER TABLE public.tiktok_inventory_sync_log
  ADD COLUMN IF NOT EXISTS sync_operation text NOT NULL DEFAULT 'receive';
...
ALTER TABLE public.tiktok_inventory_sync_log
  ...
  CHECK (sync_operation IN ('receive', 'void'));
...
-- ── log sync result (idempotent success per sync_operation) ─────────────────
DROP FUNCTION IF EXISTS public.log_tiktok_inventory_sync(bigint, bigint, text, int, int, int, text, text);
...
-- ── check prior successful sync per operation ─────────────────────────────────
DROP FUNCTION IF EXISTS public.tiktok_inventory_already_synced(bigint, bigint);
...
REVOKE ALL ON FUNCTION public.tiktok_inventory_already_synced(bigint, bigint, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.tiktok_inventory_already_synced(bigint, bigint, text) TO authenticated;

-- ── targets for void mirror (receive success, void not yet done) ────────────
...
AS $$
  ...
  LEFT JOIN public.tiktok_product_mappings m ON m.product_id = l.product_id
...
REVOKE ALL ON FUNCTION public.get_tiktok_void_mirror_targets(bigint, bigint[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_tiktok_void_mirror_targets(bigint, bigint[]) TO authenticated;
