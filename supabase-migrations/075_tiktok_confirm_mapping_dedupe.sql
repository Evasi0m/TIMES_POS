-- 075: Dedupe tiktok_sku_id before mapping upsert in confirm_tiktok_sale_order
--
-- Bug: orders with multiple sale_order_items sharing the same tiktok_sku_id
-- (common when TikTok splits qty into separate lines) caused:
--   ON CONFLICT DO UPDATE command cannot affect row a second time
-- Fix: DISTINCT ON (tiktok_sku_id) - one mapping row per SKU per confirm.
