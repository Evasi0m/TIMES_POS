-- Web shop order queue + confirm for TIMES_POS (Hybrid B).
-- Separate from get_pending_tiktok_orders / confirm_tiktok_sale_order.

CREATE OR REPLACE FUNCTION public.get_pending_web_orders(
  ...
  RETURN (
    ...
    FROM (
      SELECT
        ...
        so.payment_slip_path,
        so.payment_slip_status,
        so.buyer_name,
        so.channel,
        (
...
REVOKE ALL ON FUNCTION public.get_pending_web_orders(int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_pending_web_orders(int) TO authenticated;

CREATE OR REPLACE FUNCTION public.confirm_web_sale_order(
  p_order_id     bigint,
  p_items        jsonb,
...
BEGIN
  ...
  END IF;
  IF v_order.channel <> 'web' OR v_order.web_order_number IS NULL THEN
    RAISE EXCEPTION 'รายการนี้ไม่ใช่ออเดอร์ Web Shop' USING ERRCODE = '22023';
  END IF;
  ...
  LEFT JOIN LATERAL (
    SELECT
      COALESCE((v_entry->>'substitute')::boolean, false) AS substitute
    FROM jsonb_array_elements(COALESCE(p_items, '[]'::jsonb)) v_entry
    WHERE NULLIF(v_entry->>'item_id', '')::bigint = soi.id
    LIMIT 1
  ) pi ON true
...
REVOKE ALL ON FUNCTION public.confirm_web_sale_order(bigint, jsonb, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.confirm_web_sale_order(bigint, jsonb, numeric) TO authenticated;
