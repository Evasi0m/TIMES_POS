-- 050_tiktok_health_rpc.sql
-- get_tiktok_health — one-call admin diagnostics for the TikTok integration:
-- connection, token freshness, whether cron can authenticate (vault key),
-- the pending-confirm backlog, unmatched lines, and mapping completeness.
-- Surfaced in Settings → TikTok Shop so the owner can see at a glance whether
-- the access token is about to expire, whether background cron is enabled
-- (service_role_key in vault), and how many orders await confirmation.
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    v_key := NULL;  -- cannot read vault from this context
  END;
...
REVOKE ALL ON FUNCTION public.get_tiktok_health() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_tiktok_health() TO authenticated;
