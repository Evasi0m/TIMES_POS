...
export default function TikTokReturnModal({
  closing,
  onClose,
  onBack,
  activeOrder,
  ...
  return (
    ...
    >
      ...
      <div
        className={
          'ttc-return-modal ttc-modal-card relative w-full glass-strong border hairline overflow-hidden flex flex-col ' +
          (mobile
      ...
      >
        <div
          className={
            'ttc-modal-header ttc-amber-header ttc-header-enter relative shrink-0 ' +
            (mobile ? 'ttc-modal-header--sheet' : 'flex items-center gap-2.5 px-4 py-3')
          }
        >
          {mobile && (
          ...
          <div className={mobile ? 'flex items-center gap-2.5 px-4 pb-3 pt-1' : 'contents'}>
            {isConfirmView ? (
              ...
              <span className="ttc-amber-mesh-chip ttc-amber-mesh-chip--header">
            ...
            <div className="min-w-0 flex-1 ttc-modal-header__text">
              <div className="ttc-modal-header__title font-semibold text-[16px] leading-tight truncate">
                {isConfirmView ? TTR_COPY.modalTitle : TTR_COPY.badgeLabel}
              </div>
              <div className="ttc-modal-header__sub text-[12px] mt-0.5 tabular-nums truncate">
                {isConfirmView
                  ...
                  : TTR_COPY.modalSubtitle(count)}
              </div>
            </div>
            <button type="button" className="pnb-iconbtn" onClick={onClose} aria-label="???" disabled={saving || closing}>
              <Icon name="x" size={18}/>
            </button>
          </div>
        </div>
          ...
          {isConfirmView ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {!loading && orders.length === 0 && (
                <div className="text-center text-sm text-muted py-12">{TTR_COPY.empty}</div>
              )}
