// TikTok stock resolution ù manual return card helpers.
...
/** Client mirror of tiktok_order_was_shipped ù for labels / tests only. */
...
/** Default goods-returned radio ù null = user must choose explicitly. */
...
export function resolvedStockLabel(stockResolution) {
  switch (stockResolution) {
    case STOCK_RESOLUTION.AWAITING:
      return { label: '????????', tone: 'amber', title: '????????????????????????????????????' };
    case STOCK_RESOLUTION.RESTOCKED:
      return { label: '?????????????', tone: 'ok', title: '???????????????????????????' };
    case STOCK_RESOLUTION.LOST:
      return { label: '?????? (??????)', tone: 'warn', title: '???????????????????????????' };
  }
  return null;
}
